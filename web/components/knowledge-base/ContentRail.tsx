                ...
                <div className="flex gap-4 min-w-min">
                    {items.map((item) => (
                        ...
                        >
                            {/* Thumbnail */}
                            {item.slug ? (
                                <Link href={`/knowledge-base/articles/${item.slug}`} className="block relative h-[160px] rounded-xl overflow-hidden mb-3 group-hover:shadow-lg transition-all">
                                    <div
                                    ...
                                    {/* Premium Badge */}
                                    {item.isPremium && (
                                        <div className="absolute top-2 left-2 px-2 py-1 bg-stone-900/90 border border-stone-700 text-stone-300 text-[10px] font-bold uppercase tracking-wider rounded">
                                            Premium
                                        </div>
                                    )}
                                </Link>
                            ...
                            <div className="space-y-2">
                                <div className="flex items-start justify-between gap-2">
                                    {item.slug ? (
                                        <Link href={`/knowledge-base/articles/${item.slug}`} className="font-bold text-stone-900 dark:text-white line-clamp-2 flex-1 group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">
                                            {item.title}
                                        </Link>
                                    ) : (
